...
export default function Speaking() {
  ...
  function toggleRecord() {
    ...
    if (!isRecording) {
      ...
      // Evaluasi kemiripan dan beri reward XP satu kali
      setTimeout(() => {
        const currentText = document.getElementById("transcript-text")?.innerText || "";
        if (currentText && currentText !== "Mendengarkan... (Silakan bicara)") {
  ...
  function handleNextPrompt() {
    ...
    setTranscript("");
    setScore(null);
    if (speakingBanks && topic) {
      const bank = speakingBanks.filter(s => s.level === topic);
      const pool = bank.length > 0 ? bank : speakingBanks;
      const otherPrompts = pool.filter(p => p.id !== activePrompt?.id);
      const chosenPrompt = otherPrompts.length > 0
        ? otherPrompts[Math.floor(Math.random() * otherPrompts.length)]
        : pool[Math.floor(Math.random() * pool.length)];
      setActivePrompt(chosenPrompt);
    }
  }
    ...
    return (
      <div className="min-h-[calc(100vh-64px)]">
        <div className="max-w-[700px] mx-auto px-4 sm:px-6 py-8 pb-16">
          <header className="mb-6 pb-4 border-b border-gray-200 dark:border-slate-700 flex items-center justify-between gap-4">
            ...
            <button
              ...
              className="px-3.5 py-1.5 rounded-xl border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-slate-700 text-xs font-bold cursor-pointer transition-colors flex items-center gap-1.5"
            >
              🎲 Kalimat Lain
            </button>
          </header>
          ...
          <div className="bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-[20px] p-6 sm:p-8 mb-8 shadow-sm">
            <h2 className="text-[0.75rem] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-[1.5px] mb-3">Prompt ({activePrompt.level})</h2>
            <p className="text-[1.15rem] font-bold text-gray-900 dark:text-gray-100 leading-relaxed m-0 mb-4">{activePrompt.scenario}</p>
            
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-rose-50/60 dark:bg-rose-900/20 p-4 rounded-xl border border-rose-100 dark:border-rose-900/40">
              <p className="text-[0.95rem] text-gray-800 dark:text-gray-200 font-medium m-0">
                Target: <span className="font-bold text-rose-700 dark:text-rose-400">"{activePrompt.targetText}"</span>
              </p>
              <button 
                onClick={() => speak(activePrompt.targetText)}
                className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white dark:bg-slate-800 text-rose-700 dark:text-rose-300 hover:bg-rose-50 dark:hover:bg-slate-700 text-xs font-bold transition-colors cursor-pointer border border-rose-200 dark:border-rose-800 shrink-0 shadow-sm"
                title="Dengarkan pengucapan target"
              >
                🔊 Dengar Contoh
              </button>
            </div>
          ...
          {score !== null && (
            <div className={`border rounded-2xl p-6 text-center shadow-sm ${score >= 50 ? 'bg-green-50 dark:bg-green-900/20 border-green-200 dark:border-green-800' : 'bg-yellow-50 dark:bg-yellow-900/20 border-yellow-200 dark:border-yellow-800'}`}>
              <h3 className={`text-[1.25rem] font-extrabold mb-2 m-0 ${score >= 50 ? 'text-green-700 dark:text-green-400' : 'text-yellow-700 dark:text-yellow-400'}`}>
                Pronunciation Score: {score}/100
              </h3>
              <p className={`text-[0.9rem] m-0 mb-4 ${score >= 50 ? 'text-green-600 dark:text-green-300' : 'text-yellow-600 dark:text-yellow-300'}`}>
                {score >= 80 ? "Luar biasa! Pelafalanmu sangat jelas (+15 XP)." : score >= 50 ? "Lumayan bagus! Latih intonasi agar semakin natural (+15 XP)." : "Coba dengarkan contoh audio lalu tirukan kembali."}
              </p>
              <div className="flex items-center justify-center gap-3">
                <button 
                  onClick={() => { setTranscript(""); setScore(null); }}
                  className="bg-white dark:bg-slate-800 border border-gray-300 dark:border-slate-600 text-gray-700 dark:text-gray-200 px-5 py-2.5 rounded-full cursor-pointer font-bold text-[0.85rem] transition-colors hover:bg-gray-50"
                >
                  Ulangi Kalimat Ini
                </button>
                <button 
                  onClick={handleNextPrompt}
                  className="bg-rose-700 text-white px-5 py-2.5 rounded-full cursor-pointer border-none font-bold text-[0.85rem] transition-colors hover:bg-rose-800"
                >
                  Kalimat Berikutnya &#8594;
                </button>
              </div>
